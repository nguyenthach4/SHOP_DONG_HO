...
using DongHoShop.Common.Exceptions;
...
namespace DongHoShop.Service
{
    ...
    public class ApplicationRoleService : IApplicationRoleService
    {
        ...
        {
            if (_appRoleRepository.CheckContains(x => x.Descriptione == appRole.Descriptione))
        ...
        public IEnumerable<ApplicationRole> GetAll(int page, int pageSize, out int totalRow, string filter = null)
        {
            ...
            if (!string.IsNullOrEmpty(filter))
                query = query.Where(x => x.Descriptione.Contains(filter));

            totalRow = query.Count();
            return query.OrderBy(x => x.Descriptione).Skip(page * pageSize).Take(pageSize);
        }
        ...
        {
            if (_appRoleRepository.CheckContains(x => x.Descriptione == AppRole.Descriptione && x.Id != AppRole.Id))
                throw new NameDuplicatedException("Tên không được trùng");
